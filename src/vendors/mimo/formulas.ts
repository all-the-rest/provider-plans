...
/**
 * MiMo: Pattern-Input (fresh) → Feld `inputMiss`, Pattern-Cached → Feld `input`
 * (Cache-Hit), Output → `output`. D. h. das Credit-/API-Feld je Pattern-Feld ist
 * umgemappt (`creditPerM`/`apiPrice` nutzen `input`/`inputMiss`/`output`).
 */
const creditFieldKey = (f: CreditField): CreditField =>
  f === "input" ? "inputMiss" : f === "cached" ? "input" : f;
