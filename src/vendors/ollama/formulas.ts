import type { CreditField, Cycle, Formulas, Model, PeakConfig, Plan, VendorPriceData } from "../../types";
import {
...
/** Ollama: Pattern-Input→input, Cached→cached, Output→output (Identitäts-Mapping, kein Tier). */
export function makeFormulas(data: VendorPriceData, peak: PeakConfig, flagshipId: string): Formulas {
  ...
  const planValue = (plan: Plan, cycle: Cycle): number | null => {
    const flagship = data.models.find((mm) => mm.id === flagshipId && mm.tier === null) ?? data.models.find((mm) => mm.id === flagshipId);
    ...
    if (apiCost === null || cpr === null || pool === null || pool <= 0 || price === null || price <= 0) return null;
