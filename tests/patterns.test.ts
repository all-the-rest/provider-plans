...
test("normalizeName: lowercase + non-alnum raus", () => {
  ...
  assert.equal(normalizeName("MiMo-V2.6-Flash"), "mimov2.6flash");
  assert.equal(normalizeName("MiMo-V2.6-Pro"), "mimov2.6pro");
});
...
test("patterns: liest Fixture go.md und handschuht Komma-Dezimale (32.500 → 32500)", async () => {
  ...
  assert.deepEqual(map.get("mimov2.6flash"), { input: 830, cached: 71500, output: 295 });
  assert.deepEqual(map.get("mimov2.6pro"), { input: 790, cached: 86000, output: 305 });
...
test("patterns: Wird tiefer beim HTML-Pfad (li-Elemente) verarbeitet", () => {
  const html = `<ul>
    ...
    <li>MiMo-V2.6-Pro — 790 Input-, 86.000 Cached-, 305 Output-Tokens pro Anfrage</li>
  </ul>`;
